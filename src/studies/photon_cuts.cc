...
struct photon_cuts {
  ...
  static constexpr std::array<const char*,3> tags {
    "all", "photons_pass", "photons_pass_new"
  };
  inline static int pass;
  static void set(bool cut_all, bool cut_old, bool cut_new) noexcept {
    pass = cut_all ? 0 : ( (!cut_old)<<0 | (!cut_new)<<1 );
  }
};
template <typename Bin>
...
#define h_(NAME) \
  ...
  for (ivanp::tcnt cnt(entries_range[0],entries_range[1]); cnt; ++cnt) {
    ...
    photon_cuts::set(
      ...
      (photon_pt[1] < 0.25*mH),
      (std::sqrt(photon_pt[0]*photon_pt[1]) < 0.35*mH) or
      (photon_pt[1] < 0.25*mH)
    );
    ...
    h_pT_y1(photon_pt[0]);
    h_pT_y2(photon_pt[1]);
    h_pT_y1_frac(photon_pt[0]/pT_yy);
    h_pT_y2_frac(photon_pt[1]/pT_yy);

    const auto absy_y = photons | [](const auto& p){ return std::abs(p.rap()); };
    const double absy_yy = std::abs(higgs.rap());
